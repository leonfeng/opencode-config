...
| `stop-think-loop` | Recovers when a reply hits the output token limit while still thinking: strips stalled reasoning, disables thinking, and prompts a visible reply. Re-recovers after productive turns (sentinel stays in history but no longer blocks). **BigBang** build agent only: thinking disabled proactively with 8k output cap (Qwen and other reasoning models keep thinking). Once a session stalls, output drops to 4k for the rest of the session. Strips spent reasoning from tool/text turns. Caps compaction output, turns off overflow autocontinue, and merges extra system prompts so Qwen/vLLM does not 400 on title generation. |
...
| `stop-agent-loop` | **Generic** stuck-session guard: blocks the 3rd identical tool call (git diff, read, edit, grep, pytest, …). No-progress (20 primary / 12 child) counts verify steps only — first-pass read/grep/glob/`openspec` and skill/task/question do not count. After the cap, write/edit/question still run. `@explore` children may look up 20 unique files/patterns, then must close with findings. `# confirm` works on any tool (append to command or path). |
