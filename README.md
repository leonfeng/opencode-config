...
- **Also configured (not currently served):** KAT-Coder V2.5, Gemma 4 26B-A4B
...
| `propose-ask-first` | When `/opsx-propose` runs with no name/description, injects an ask-first system hint and blocks tools (except `question`) until the user describes the change. Append `# confirm` to override. |
| `stop-think-loop` | Recovers when a reply hits the output token limit while still thinking, or when a long think block ends with almost no tools/visible text (`finish=unknown` step timeout): strips stalled reasoning, disables thinking, and prompts a visible reply. **BigBang build/apply:** thinking disabled proactively with 8k output cap; explore/subagents keep thinking. Once a session stalls, output drops to 4k for the rest of the session. Strips spent reasoning from tool/text turns. Caps compaction output, turns off overflow autocontinue, and merges extra system prompts so vLLM does not 400 on title generation. |
...
- **`no-rewrite-loop.md`** — after a successful write/edit, mark the OpenSpec task done; do not rewrite existing files or rerun finished commands; use OpenCode tools instead of shell stand-ins; empty `/opsx-propose` must ask before exploring; explore/archive stop conditions.
